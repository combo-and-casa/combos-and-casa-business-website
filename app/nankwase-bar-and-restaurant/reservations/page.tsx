  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const emailResponse = await fetch('/api/send-reservation-confirmation', {
        ...
        body: JSON.stringify({
          reservationId: reservationInsert.id,
          name: formData.name,
          email: formData.email,
          phone: formData.phone,
          date: formData.date,
      ...
      if (emailResult.success) {
        ...
        console.warn('⚠️ Email sending failed, but reservation is successful');
      }
    } catch (emailError) {
      console.error('⚠️ Email error (reservation still successful):', emailError);
    }

    toast.dismiss();
    toast.success('Reservation submitted successfully! Check your email for confirmation.');
